...
import {
  ...
  GUA_SERIAL_SONG,
  HEART_SONG,
} from './scripture-content.constant';
...
export class FullGuaFactory {
  ...
  private readonly MONSTERS_ARRAY = [
    '青龍',
    '朱雀',
    '勾陳',
    '呈蛇',
  ...
  create({ up, down, mutual, date, cutAt2300, thing }: CreateParams): FullGua {
    ...
    let fullGua: FullGua = new FullGua(
      '乾為天',
      '旱象逢河',
      this.genSixYao(down, up, FullGuaFactory.getGungElement('乾')),
      FullGuaFactory.genHeavenlyStems(down, up),
      FullGuaFactory.genGung('乾'),
      undefined,
      this.collisionHint,
    );
    switch (up) {
      case '天':
        if (down === '天') {
          ...
          fullGua = new FullGua(
            '天澤履',
            '行走薄冰',
            this.genSixYao(down, up, FullGuaFactory.getGungElement('艮')),
            FullGuaFactory.genHeavenlyStems(down, up),
            FullGuaFactory.genGung('艮'),
          ...
          fullGua = new FullGua(
            '天火同人',
            '仙人指路',
            this.genSixYao(down, up, FullGuaFactory.getGungElement('離')),
            FullGuaFactory.genHeavenlyStems(down, up),
            FullGuaFactory.genGung('離'),
            undefined,
            this.returnHint,
          );
        } else if (down === '雷') {
          fullGua = new FullGua(
            '天雷無妄',
            '宜守本分',
            this.genSixYao(down, up, FullGuaFactory.getGungElement('巽')),
            FullGuaFactory.genHeavenlyStems(down, up),
            FullGuaFactory.genGung('巽'),
            undefined,
            this.collisionHint,
          );
        } else if (down === '風') {
      ...
      case '澤':
        if (down === '天') {
          fullGua = new FullGua(
            '澤天夬',
            '遊蜂脫網',
            this.genSixYao(down, up, FullGuaFactory.getGungElement('坤')),
            FullGuaFactory.genHeavenlyStems(down, up),
            FullGuaFactory.genGung('坤'),
          ...
          fullGua = new FullGua(
            '兌為澤',
            '趁水和泥',
            this.genSixYao(down, up, FullGuaFactory.getGungElement('兌')),
            FullGuaFactory.genHeavenlyStems(down, up),
            FullGuaFactory.genGung('兌'),
            undefined,
            this.collisionHint,
          );
        } else if (down === '火') {
          fullGua = new FullGua(
            '澤火革',
            '憂去喜來',
            this.genSixYao(down, up, FullGuaFactory.getGungElement('坎')),
            FullGuaFactory.genHeavenlyStems(down, up),
            FullGuaFactory.genGung('坎'),
          ...
          fullGua = new FullGua(
            '澤雷隨',
            '推車靠崖',
            this.genSixYao(down, up, FullGuaFactory.getGungElement('震')),
            FullGuaFactory.genHeavenlyStems(down, up),
            FullGuaFactory.genGung('震'),
            [{ earthlyBranch: '午', relative: '子孫', position: 4 }],
            this.returnHint,
          );
        } else if (down === '風') {
          fullGua = new FullGua(
      ...
      case '火':
        if (down === '天') {
          ...
          fullGua = new FullGua(
            ...
            FullGuaFactory.genGung('離'),
            [
              { earthlyBranch: '卯', relative: '父母', position: 1 },
              { earthlyBranch: '亥', relative: '官鬼', position: 3 },
            ],
            this.suitHint,
          );
      ...
      case '山':
        if (down === '天') {
          fullGua = new FullGua(
            ...
            FullGuaFactory.genGung('艮'),
            [
              { earthlyBranch: '午', relative: '父母', position: 2 },
              { earthlyBranch: '申', relative: '子孫', position: 3 },
            ],
          );
        } else if (down === '澤') {
  ...
  private genSixYao(down: Gua, up: Gua, gungElement: Elements): Yao[] {
    const earthlyBranches = [
      ...FullGuaFactory.getEarthlyBranch(down, 'DOWN'),
      ...FullGuaFactory.getEarthlyBranch(up, 'UP'),
    ];
    const isYangYao = [
      ...FullGuaFactory.genIsYangYao(down),
      ...FullGuaFactory.genIsYangYao(up),
    ];
    return earthlyBranches.map((e, i) => ({
      earthlyBranch: e,
      relative: this.getRelative(gungElement, e),
      position: i + 1,
      isYangYao: isYangYao[i],
      void: false,
    }));
  }
  ...
  private genDate(fullGua: FullGua, date: Date, cutAt2300: boolean): void {
    ...
    fullGua.void = this.calculateVoid(
      fullDate.lunar.getDayGan() as HeavenlyStem,
      fullDate.lunar.getDayZhi() as EarthlyBranch,
    );
   ...
   * @param d 日期
   */
  private static transLunarDate(d: Date): {
    lunarDate: string;
    solarDate: string;
    lunar: Lunar;
  } {
  ...
  private genMutual(up: Gua, down: Gua, mutual: number[], fullGua: FullGua) {
    ...
    if (mutual.length > 0) {
      fullGua.mutual = mutual.map((n) => ({
        ...
        relative: this.getRelative(
          fullGua.gung.element,
          mutualFullGua.yao[n - 1].earthlyBranch,
        ),
        void: false,
      }));
    }
    ...
    if (mutual.length > 0) {
      const guaWord = guaWords.find(
        (g) => g.guaIndex === mutualFullGua.originalName,
      );
      fullGua.genGuaMeaningMutual(
        `${guaWord?.guaMean}${guaWord?.guaMeanDetail}`,
      );
    }
    if (
      fullGua.hints.find((h) => h.includes('六合卦')) &&
      mutualFullGua.hints.find((h) => h.includes('六沖卦'))
    ) {
      fullGua.addHint('大卦合化沖');
    }
    if (
      fullGua.hints.find((h) => h.includes('六沖卦')) &&
      mutualFullGua.hints.find((h) => h.includes('六合卦'))
    ) {
  ...
  private addMutualScripture(fullGua: FullGua, mutual: number[]): void {
    ...
    if (!guaWord) {
      throw new Error(
        `找不到經書(addScriptures)，卦名：${fullGua.originalName}`,
      );
    }
    if (mutual && mutual.length !== 0) {
      ...
      const mutualContent = mutual
        ...
        .join('\n');
      fullGua.addScripture({
        title: '動爻解析',
        content: mutualContent,
        scripture: fullYaoContent,
      });
    }
  }
  ...
  private addScriptures(fullGua: FullGua): void {
    ...
    fullGua.addScripture({
      title: '五路財神經',
      content: guaWord.fiveMoney,
      scripture: GOD_OF_WEALTH,
    });
    fullGua.addScripture({
      title: '稽首七十二天師加持世界和平共轉法輪寶誥',
      content: guaWord.seventyTwoGod.join(''),
      scripture: GOD_72,
    });
    fullGua.addScripture({
      title: '序卦傳',
      content: guaWord.serialGua,
      scripture: GUA_SERIAL_SONG,
    });
    fullGua.addScripture({
      title: '唯心用易歌訣',
      content: guaWord.heartSong,
      scripture: HEART_SONG,
    });
  }
   ...
   * @return 填寫地支
   */
  private static getEarthlyBranch(
    gua: Gua,
    position: 'UP' | 'DOWN',
  ): EarthlyBranch[] {
    let earthlyBranches!: EarthlyBranch[];
  ...
  private calculateVoid(
    ...
    return [
      this.EARTHLY_BRANCHES[voidBegin],
      this.EARTHLY_BRANCHES[voidBegin + 1],
    ];
  }
  ...
  private getMutualFullGua(up: Gua, down: Gua, mutual: number[]): FullGua {
    let mutualGuaYingYang =
      FullGuaFactory.transGuaToYingYangYao(down) +
      FullGuaFactory.transGuaToYingYangYao(up);

    for (const n of mutual) {
      switch (mutualGuaYingYang.charAt(n - 1)) {
        case '0':
          mutualGuaYingYang = FullGuaFactory.replaceAt(
            mutualGuaYingYang,
            n - 1,
            '1',
          );
    ...
    const mutualDown = this.transYingYangYaoToGua(
      mutualGuaYingYang.substring(0, 3) as YingYangYao,
    );
    const mutualUp = this.transYingYangYaoToGua(
      mutualGuaYingYang.substring(3, 6) as YingYangYao,
    );
