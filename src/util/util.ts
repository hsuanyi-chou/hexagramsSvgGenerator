 ...
 */
export const isDarkMutual = ({
  yao,
  lunarMonth,
  lunarDay,
}: {
  yao: Yao;
  lunarMonth: EarthlyBranch;
  lunarDay: EarthlyBranch;
}): boolean => {
  if (!isMonthWang({ target: yao.earthlyBranch, lunarMonth })) {
    return false;
  }
  const day = earthlyBranchDay({
    target: yao.earthlyBranch,
    compare: lunarDay,
    handle12LongLife: { variant: '月', month: lunarMonth },
  });
  if (!day.includes('沖')) {
    return false;
  }
